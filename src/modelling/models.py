...
class CatBoostNet:
    def __init__(self):
        ...
        self.model = self.__build()
        self.feature_importance = None  # Attribute to store feature importance
    ...
    def get_feature_importance(self, feature_names=None, plot=True):
        """
        Get and optionally plot feature importance.

        Parameters:
            feature_names (list): List of feature names for better readability.
            plot (bool): Whether to display a bar plot of feature importance.

        Returns:
            feature_importance (list): Feature importance scores.
        """
        if self.feature_importance is None:
            print("Feature importance is not available. Train the model first.")
            return None

        # Map feature importance to feature names if provided
        if feature_names:
            importance_dict = dict(zip(feature_names, self.feature_importance))
            sorted_importance = sorted(importance_dict.items(), key=lambda x: x[1], reverse=True)
        else:
            sorted_importance = list(enumerate(self.feature_importance))
